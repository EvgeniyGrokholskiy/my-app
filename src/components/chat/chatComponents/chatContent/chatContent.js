...
export const ChatContent = (props) => {
    ...
    let chatToRender = props.chatMessage.map((message) => (message.type === "in")? <IncomingMessage key={message.id} message = {message.message} />: <OutgoingMessage key={message.id} message = {message.message} />)
