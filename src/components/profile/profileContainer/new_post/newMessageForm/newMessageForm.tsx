...
import {INewMessageForm} from "../../../../../types/types"
import {ReactComponent as SendIcon} from "../img/send_icon.svg"


const NewMessageForm: React.FC<INewMessageForm> = React.memo(({addPost}) => {

    const handleAddPost = (data: { newMessage: string }) => {
        if (data.newMessage) addPost(data.newMessage);
    }

    const maxLength300 = validators.maxLengthValidatorCreator(300);
    return (
        <Form
            onSubmit={(data: { newMessage: string }) => {
                handleAddPost(data)
            }}
